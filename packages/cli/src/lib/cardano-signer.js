...
const parse_options = {
  ...
  boolean: [
    ...
    "include-secret",
    "signature-only",
  ], //all booleans are set to false per default
  ...
  unknown: function (unknownParameter) {
    ...
    if (!numberParams.includes(unknownParameter.substring(2).toLowerCase())) {
      //throw an error if given parameterName is not in any of the lists above
      process.stderr.write(`Error: Unknown parameter '${unknownParameter}'`);
      if (!unknownParameter.startsWith("--")) {
        process.stderr.write(` - parameters must start with a double hypen like --secret-key`);
      }
...
function showUsage(topic, exit = true) {
  ...
  switch (topic) {
    case "sign":
      ...
      console.log(
        `   Params: ${FgGreen}--data-hex${Reset} "<hex>" | ${FgGreen}--data${Reset} "<text>" | ${FgGreen}--data-file${Reset} "<path_to_file>"`,
      );
      console.log(`								${Dim}data/payload/file to sign in hex-, text- or binary-file-format${Reset}`);
      console.log(
        `           ${FgGreen}--secret-key${Reset} "<path_to_file>|<hex>|<bech>"		${Dim}path to a signing-key-file or a direct signing hex/bech-key string${Reset}`,
      );
      console.log(
        `           [${FgGreen}--address${Reset} "<path_to_file>|<hex>|<bech>"]		${Dim}optional address check against the signing-key (address-file or a direct bech/hex format)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--include-secret${Reset}]					${Dim}optional flag to include the secret/signing key in the json-extended output${Reset}`,
      );
      console.log(
        `           [${FgGreen}--signature-only${Reset}]					${Dim}optional flag to only output the signature in std-output without the publicKey${Reset}`,
      );
      console.log(
        `           [${FgGreen}--json${Reset} |${FgGreen} --json-extended${Reset}]				${Dim}optional flag to generate output in json/json-extended format${Reset}`,
      );
      console.log(
        `           [${FgGreen}--jcli${Reset} |${FgGreen} --bech${Reset}]					${Dim}optional flag to generate signature & publicKey in jcli compatible bech-format${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-file${Reset} "<path_to_file>"]			${Dim}path to an output file, default: standard-output${Reset}`,
      );
      console.log(
        `   Output: ${FgCyan}"signature + publicKey"${Reset} or ${FgCyan}JSON-Format${Reset}		${Dim}default: hex-format${Reset}`,
      );
      console.log(``);
      break;
    ...
    case "sign-cip30":
      ...
      console.log(
        `   Params: ${FgGreen}--data-hex${Reset} "<hex>" | ${FgGreen}--data${Reset} "<text>" | ${FgGreen}--data-file${Reset} "<path_to_file>"${Reset}`,
      );
      console.log(`								${Dim}data/payload/file to sign in hex-, text- or binary-file-format${Reset}`);
      console.log(
        `           ${FgGreen}--secret-key${Reset} "<path_to_file>|<hex>|<bech>"		${Dim}path to a signing-key-file or a direct signing hex/bech-key string${Reset}`,
      ...
      console.log(
        `           ${FgGreen}--address${Reset} "<path_to_file>|<hex>|<bech>"		${Dim}path to an address-file or a direct bech/hex format 'stake1..., stake_test1..., addr1...'${Reset}`,
      );
      console.log(
        `           [${FgGreen}--nohashcheck${Reset}]					${Dim}optional flag to not perform a check that the public-key belongs to the address/hash${Reset}`,
      );
      console.log(
        `           [${FgGreen}--hashed${Reset}]						${Dim}optional flag to hash the payload given via the 'data' parameters${Reset}`,
      );
      console.log(
        `           [${FgGreen}--nopayload${Reset}]					${Dim}optional flag to exclude the payload from the COSE_Sign1 signature, default: included${Reset}`,
      );
      console.log(
        `           [${FgGreen}--testnet-magic [xxx]${Reset}]				${Dim}optional flag to switch the address check to testnet-addresses, default: mainnet${Reset}`,
      );
      console.log(
        `           [${FgGreen}--include-maps${Reset}]					${Dim}optional flag to include the COSE maps in the json-extended output${Reset}`,
      );
      console.log(
        `           [${FgGreen}--include-secret${Reset}]					${Dim}optional flag to include the secret/signing key in the json-extended output${Reset}`,
      );
      console.log(
        `           [${FgGreen}--signature-only${Reset}]					${Dim}optional flag to only output the COSE_Sign1 in std-output without the COSE_Key${Reset}`,
      );
      console.log(
        `           [${FgGreen}--json${Reset} |${FgGreen} --json-extended${Reset}]				${Dim}optional flag to generate output in json/json-extended format${Reset}`,
      );
    ...
    case "sign-cip36-deregister":
      console.log(``);
      console.log(
        `${Bright}${Underscore}Sign and generate a catalyst registration/delegation or deregistration in CIP-36 mode:${Reset}`,
      );
      console.log(``);
      console.log(`   Syntax: ${Bright}${appname} ${FgGreen}sign --cip36${Reset}`);
      console.log(
        `   Params: [${FgGreen}--vote-public-key${Reset} "<path_to_file>|<hex>|<bech>"	${Dim}public-key-file(s) or public hex/bech-key string(s) to delegate the votingpower to (single or multiple)${Reset}`,
      );
      console.log(
        `           ${FgGreen}--vote-weight${Reset} <unsigned_int>]			${Dim}relative weight of each delegated votingpower, default: 100% for a single delegation${Reset}`,
      );
      console.log(
        `           ${FgGreen}--secret-key${Reset} "<path_to_file>|<hex>|<bech>"		${Dim}signing-key-file or a direct signing hex/bech-key string of the stake key (votingpower)${Reset}`,
      );
      console.log(
        `           ${FgGreen}--payment-address${Reset} "<path_to_file>|<hex>|<bech>"	${Dim}rewards payout address (address-file or a direct bech/hex format 'addr1..., addr_test1...')${Reset}`,
      );
      console.log(
        `           [${FgGreen}--nonce${Reset} <unsigned_int>]				${Dim}optional nonce value, if not provided the mainnet-slotHeight calculated from current machine-time will be used${Reset}`,
      );
      console.log(
        `           [${FgGreen}--vote-purpose${Reset} <unsigned_int>]			${Dim}optional parameter (unsigned int), default: 0 (catalyst)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--deregister${Reset}]					${Dim}optional flag to generate a deregistration (no --vote-public-key/--vote-weight/--payment-address needed${Reset}`,
      );
      console.log(
        `           [${FgGreen}--testnet-magic [xxx]${Reset}]				${Dim}optional flag to switch the address check to testnet-addresses, default: mainnet${Reset}`,
      );
      ...
      console.log(
        `           [${FgGreen}--json${Reset} |${FgGreen} --json-extended${Reset}]				${Dim}optional flag to generate output in json/json-extended format, default: cborHex(text)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-file${Reset} "<path_to_file>"]			${Dim}path to an output file, default: standard-output${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-cbor${Reset} "<path_to_file>"]			${Dim}path to write a binary metadata.cbor file to${Reset}`,
      );
    ...
    case "sign-cip88":
      console.log(``);
      console.log(
        `${Bright}${Underscore}Sign and generate a Calidus-Pool-PublicKey registration with a Pool-Cold-Key in CIP-88v2 mode:${Reset}`,
      );
      console.log(``);
      console.log(`   Syntax: ${Bright}${appname} ${FgGreen}sign --cip88${Reset}`);
      console.log(
        `   Params: ${FgGreen}--calidus-public-key${Reset} "<path_to_file>|<hex>|<bech>"	${Dim}public-key-file or public hex/bech-key string to use as the new calidus-key${Reset}`,
      );
      console.log(
        `           ${FgGreen}--secret-key${Reset} "<path_to_file>|<hex>|<bech>"		${Dim}signing-key-file or a direct signing hex/bech-key string of the stakepool${Reset}`,
      );
      console.log(
        `           [${FgGreen}--nonce${Reset} <unsigned_int>]				${Dim}optional nonce value, if not provided the mainnet-slotHeight calculated from current machine-time will be used${Reset}`,
      );
    ...
    case "sign-cip100":
      console.log(``);
      console.log(
        `${Bright}${Underscore}Sign a governance JSON-LD metadata file with a Secret-Key (add authors, ed25519/CIP-8 algorithm):${Reset}`,
      );
      console.log(``);
      console.log(`   Syntax: ${Bright}${appname} ${FgGreen}sign --cip100${Reset}`);
      console.log(
        `   Params: ${FgGreen}--data${Reset} "<jsonld-text>" | ${FgGreen}--data-file${Reset} "<path_to_jsonld_file>"${Reset}`,
      );
      ...
      console.log(
        `           ${FgGreen}--author-name${Reset} "<name-of-signing-author>"		${Dim}name of the signing author f.e. "John Doe"${Reset}`,
      );
      console.log(
        `           [${FgGreen}--address${Reset} "<path_to_file>|<hex>|<bech>"]		${Dim}optional path to an address/id-file or a direct bech/hex format 'stake1..., addr1..., drep1...' to sign with CIP-8 algorithm${Reset}`,
      );
      console.log(
        `           [${FgGreen}--replace${Reset}]						${Dim}optional flag to replace the authors entry with the same public-key${Reset}`,
      );
      console.log(
        `           [${FgGreen}--disable-safemode${Reset}]					${Dim}optional flag to disable the safe-mode for the jsonld canonization${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-file${Reset} "<path_to_file>"]			${Dim}path to an output file, default: standard-output${Reset}`,
      );
      console.log(
        `   Output: ${FgCyan}"Signed JSON-LD Content"${Reset} or ${FgCyan}"JSON-HashInfo if --out-file is used"${Reset}`,
      );
      console.log(``);
      break;
    ...
    case "verify":
      ...
      console.log(
        `           ${FgGreen}--signature${Reset} "<hex>|<bech>"				${Dim}signature in hex- or bech-format${Reset}`,
      );
      console.log(
        `           ${FgGreen}--public-key${Reset} "<path_to_file>|<hex>|<bech>"		${Dim}path to a public-key-file or a direct public hex/bech-key string${Reset}`,
      );
      console.log(
        `           [${FgGreen}--address${Reset} "<path_to_file>|<hex>|<bech>"]		${Dim}optional address check against the public-key (address-file or a direct bech/hex format)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--json${Reset} |${FgGreen} --json-extended${Reset}]				${Dim}optional flag to generate output in json/json-extended format${Reset}`,
      );
    ...
    case "verify-cip30":
      ...
      console.log(
        `   Params: ${FgGreen}--cose-sign1${Reset} "<hex>"					${Dim}COSE_Sign1 signature in cbor-hex-format${Reset}`,
      );
      console.log(
        `           ${FgGreen}--cose-key${Reset} "<hex>"					${Dim}COSE_Key containing the public-key in cbor-hex-format${Reset}`,
      );
      console.log(
        `           [${FgGreen}--data-hex${Reset} "<hex>" | ${FgGreen}--data${Reset} "<text>" | ${FgGreen}--data-file${Reset} "<path_to_file>"${Reset}]`,
      );
      console.log(`								${Dim}optional data/payload/file if not present in the COSE_Sign1 signature${Reset}`);
      console.log(
        `           [${FgGreen}--address${Reset} "<path_to_file>|<hex>|<bech>"]		${Dim}optional signing-address to do the verification with${Reset}`,
      );
      console.log(
        `           [${FgGreen}--nohashcheck${Reset}]					${Dim}optional flag to not perform a check that the public-key belongs to the address/hash${Reset}`,
      );
    ...
    case "verify-cip88":
      ...
      console.log(
        `           ${FgGreen}--data-file${Reset} "<path_to_file>" |			${Dim}data to verify as json file${Reset}`,
      );
    ...
    case "verify-cip100":
      console.log(``);
      console.log(
        `${Bright}${Underscore}Verify Signatures in CIP-100/108/119/136 governance JSON-LD metadata:${Reset}`,
      );
      console.log(``);
    ...
    case "keygen-exodus":
      ...
      console.log(
        `   Params: [${FgGreen}--path${Reset} "<derivationpath>"]				${Dim}optional derivation path in the format like "1852H/1815H/0H/0/0" or "1852'/1815'/0'/0/0"${Reset}`,
      );
      console.log(
        `								${Dim}or predefined names: --path payment, --path stake, --path cip36, --path drep, --path cc-cold,${Reset}`,
      );
      console.log(`								${Dim}                     --path cc-hot, --path pool, --path calidus${Reset}`);
      console.log(
        `           [${FgGreen}--mnemonics${Reset} "word1 word2 ... word24" 		${Dim}optional mnemonic words to derive the key from (separate via space)${Reset}`,
      );
      console.log(
        `            ${FgGreen}--mnemonics${Reset} <path_to_mnemonics_file>]		${Dim}optional mnemonic words provided via a text-file${Reset}`,
      );
      console.log(
        `           [${FgGreen}--passphrase${Reset} "passphrase"] 				${Dim}optional passphrase for --ledger or --trezor derivation method${Reset}`,
      );
      console.log(
        `           [${FgGreen}--ledger | --trezor |${Reset} 				${Dim}optional flag to set the derivation type to "Ledger" or "Trezor" hardware wallet${Reset}`,
      );
      console.log(
        `            ${FgGreen}--byron | --yoroi |${Reset}					${Dim}optional flag to set the derivation type to "Daedalus(Byron 12/27 words)" or "Yoroi(Byron 15/21 words)"${Reset}`,
      );
      console.log(
        `            ${FgGreen}--exodus | --exodus-stake${Reset}]				${Dim}optional flag to set the derivation type to "Exodus(Shelley)-Payment" or "Exodus(Shelley)-Stake"${Reset}`,
      );
      console.log(
        `           [${FgGreen}--address${Reset} "<path_to_file>|<hex>|<base58>"]		${Dim}optional byron address (DDz...) in --byron mode to do an auto-derivation-path discovery${Reset}`,
      );
      console.log(
        `           [${FgGreen}--cip36${Reset}] 						${Dim}optional flag to generate CIP36 conform vote keys (also using path 1694H/1815H/0H/0/0)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--vote-purpose${Reset} <unsigned_int>]			${Dim}optional vote-purpose (unsigned int) together with --cip36 flag, default: 0 (Catalyst)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--vkey-extended${Reset}] 					${Dim}optional flag to generate a 64byte publicKey with chain code${Reset}`,
      );
      console.log(
        `           [${FgGreen}--json${Reset} |${FgGreen} --json-extended${Reset}]				${Dim}optional flag to generate output in json/json-extended format${Reset}`,
      );
      ...
      console.log(
        `           [${FgGreen}--out-skey${Reset} "<path_to_skey_file>"]			${Dim}path to an output skey-file (writes out a typical *.skey json)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-vkey${Reset} "<path_to_vkey_file>"]			${Dim}path to an output vkey-file (writes out a typical *.vkey json)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-id${Reset} "<path_to_id_file>"]			${Dim}path to an output id-file (writes out the bech-id of a pool, drep, calidus,...)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-mnemonics${Reset} "<path_to_id_file>"]		${Dim}path to an output mnemonics-file (writes out the used mnemonics into a file)${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-addr${Reset} "<path_to_addr_file>"]			${Dim}path to an output addr-file (writes out an available address)${Reset}`,
      );
      console.log(
        `   Output: ${FgCyan}"secretKey + publicKey"${Reset} or ${FgCyan}JSON-Format${Reset}		${Dim}default: hex-format${Reset}`,
      );
      console.log(``);
      break;
    ...
    case "canonize-cip100":
      console.log(``);
      console.log(
        `${Bright}${Underscore}Canonize&Hash the governance JSON-LD body metadata for author-signatures:${Reset} (CIP-100)`,
      );
      console.log(``);
      ...
      console.log(
        `           [${FgGreen}--out-canonized${Reset} "<path_to_file>"]			${Dim}path to an output file for the canonized data${Reset}`,
      );
      console.log(
        `           [${FgGreen}--out-file${Reset} "<path_to_file>"]			${Dim}path to an output file, default: standard-output${Reset}`,
      );
      console.log(
        `   Output: ${FgCyan}"HASH of canonized body"${Reset} or ${FgCyan}JSON-Format${Reset}		${FgRed}NOTE: This is NOT the anchor-url-hash!!!${Reset}`,
      );
      console.log(``);
      break;
...
function wordCount(s) {
  return s.split(" ").filter(function (n) {
    return n != "";
  }).length;
}
...
function trimMnemonic(s) {
  s = s.replace(/(^\s*)|(\s*$)/gi, ""); // exclude start and end white-space
  s = s.replace(/[ ]{2,}/gi, " "); // reduce 2 or more space between words to 1
  s = s.replace(/\n /, "\n"); // exclude newline with a start spacing
  return s;
}

function readKey2hex(key, type, jsonSearchArray) {
  //reads a standard-cardano-skey/vkey-file-json, a direct hex entry or a bech-string  // returns a hexstring of the key
  ...
  switch (type) {
    case "secret": //convert a secret key into a hex string, always returns the full privat-key-hex (extended or non-extended)
      ...
      if (!jsonSearchArray) {
        jsonSearchArray = ["Signing", "ExtendedSigning"];
      }
      ...
      try {
        ...
        const is_signing_key = jsonSearchArray.some((element) =>
          key_json.type.toLowerCase().includes(element.toLowerCase()),
        ); //boolean if the json contains one of the keywords in the jsonSearchArray
        if (!is_signing_key) {
          console.error(
            `Error: The file '${key}' is not a secret key json of type '${jsonSearchArray}'. Instead it is of type '` +
              key_json.type +
              `'`,
          );
          process.exit(1);
        ...
        if (!regExpHex.test(key_hex)) {
          console.error(`Error: The secret key in file '${key}' entry 'cborHex' is not a valid hex string`);
          process.exit(1);
        }
        return key_hex;
      } catch (error) {}
      ...
      try {
        ...
        try {
          //try to load it as a bech secret key
          key_hex = Buffer.from(bech32.fromWords(bech32.decode(content, 1000).words)).toString("hex");
        } catch (error) {
          console.error(`Error: The content in file '${key}' is not a valid bech secret key`);
          process.exit(1);
        }
        return key_hex;
      } catch (error) {}
      ...
      //check that the given key is a hex string
      if (!regExpHex.test(key)) {
        console.error(
          `Error: Provided secret key '${key}' is not a valid secret key. Or not a hex string, bech encoded key, or the file is missing`,
        );
        process.exit(1);
      }
      return key_hex;
    ...
    case "public": //convert a public key into a hex string, always return a non-extended public-key-hex
      // set the default searchstring for the type file in json key files
      if (!jsonSearchArray) {
        jsonSearchArray = ["Verification", "ExtendedVerification"];
      }

      // try to use the parameter as a filename for a cardano vkey json with a cborHex entry
      try {
        const key_json = JSON.parse(fs.readFileSync(key, "utf8")); //parse the given key as a json file
        const is_verification_key = jsonSearchArray.some((element) =>
          key_json.type.toLowerCase().includes(element.toLowerCase()),
        ); //boolean if the json contains one of the keywords in the jsonSearchArray
        if (!is_verification_key) {
          console.error(
            `Error: The file '${key}' is not a public key json of type '${jsonSearchArray}'. Instead it is of type '` +
              key_json.type +
              `'`,
          );
          process.exit(1);
        ...
        //check that the given key is a hex string
        if (!regExpHex.test(key_hex)) {
          console.error(`Error: The public key in file '${key}' entry 'cborHex' is not a valid hex string`);
          process.exit(1);
        }
      ...
      try {
        const content = trimString(fs.readFileSync(key, "utf8")); //read the content of the given key from a file
        try {
          //try to load it as a bech public key
          key_hex = Buffer.from(bech32.fromWords(bech32.decode(content, 1000).words)).toString("hex");
        } catch (error) {
          console.error(`Error: The content in file '${key}' is not a valid bech public key`);
          process.exit(1);
        }
        return key_hex.substring(0, 64); //return a non-extended public key
      } catch (error) {}
      ...
      if (!regExpHex.test(key)) {
        console.error(
          `Error: Provided public key '${key}' is not a valid hex string, bech encoded key, or the file is missing`,
        );
        process.exit(1);
      } else if (key_hex.length < 64) {
        console.error(`Error: Provided hex public key '${key}' is too short`);
        process.exit(1);
      }
      return key_hex.substring(0, 64); //return a non-extended public key
      break;
...
function readAddr2hex(addr, publicKey) {
  //reads a cardano address from a file (containing a bech address), a direct hex entry or a bech-string  // returns a hexstring of the address + type + network
  ...
  try {
    ...
    try {
      ...
      if (content.startsWith("drep") && addr_hex.length == 56) {
        addr_type = "drep";
      } else if (content.startsWith("drep_script")) {
        addr_type = "drep script";
      } else if (content.startsWith("cc_cold") && addr_hex.length == 56) {
        addr_type = "committee-cold";
      } else if (content.startsWith("cc_cold_script")) {
        addr_type = "committee-cold script";
      } else if (content.startsWith("cc_hot") && addr_hex.length == 56) {
        addr_type = "committee-hot";
      } else if (content.startsWith("cc_hot_script")) {
        addr_type = "committee-hot script";
      }
    } catch (error) {
      console.error(`Error: The address in file '${addr}' is not a valid bech address`);
      process.exit(1);
    }
  } catch (error) {}
  ...
  if (!addr_hex) {
    ...
    if (!regExpHex.test(addr_hex)) {
      console.error(
        `Error: Provided address '${addr}' is not a valid hex string, bech encoded address, or the file is missing`,
      );
      process.exit(1);
    }
  }
...
function generateLedgerMasterKey(mnemonic, passphrase) {
  ...
  const cc = crypto.createHmac("sha256", "ed25519 seed").update(message).digest();
...
function hmacRecursive(message) {
  var hmac = crypto.createHmac("sha512", "ed25519 seed").update(message).digest();
...
function generateByronMasterKeyFromMnemonic(mnemonic) {
  ...
  try {
    ...
    throw `Could not calculate entropy from mnemonic '${error}'`;
  }
...
function generateByronMasterKeyFromSeed(seed, iteration = 1) {
  ...
  const cc = crypto.createHmac("sha512", Buffer.from(seed, "hex")).update(message).digest();
  ...
  const tweakedHash = crypto.createHash("sha512").update(iL).digest();
  tweakedHash[0] &= 0b1111_1000; // clear the lowest 3 bits
  tweakedHash[31] &= 0b0111_1111; // clear the highest bit
  tweakedHash[31] |= 0b0100_0000; // set the 2nd highest bit
...
function deriveByronDaedalus(prvKeyHex, index) {
  ...
  if (index < 0x80000000) {
    throw "Byron derivation path must only contain hardened indexes";
  } // error if soft indexes
  ...
  const kLeftBuffer = Buffer.from(
    leftBigInt
      .toString(16)
      .padStart(32 * 2, "0")
      .slice(0, 32 * 2),
    "hex",
  ).reverse(); // convert the BigInt back into an 32 byte long Buffer (little endian)
  ...
  const rightArray = new Uint8Array(32).map((x, index) => {
    return (zRightArray[index] + kRightArray[index]) & 0xff;
  }); //  new private key right part is: sum of each byte in zRightArray & kRightArray. only use lowest 8 bit (0xFF)
...
function generateByronDaedalusAddress(rootPubKeyHex, pubKeyExtHex, derivationPath) {
  ...
  pathArray.forEach((element, index) => {
    ...
    if (element[element.length - 1] != "H") {
      throw "Byron derivation path must only contain hardened elements";
    }
  ...
  try {
    ((cipher = crypto.createCipheriv("chacha20-poly1305", key, nonce, {
      ...
      (encrypted = Buffer.concat([cipher.update(data, "hex"), cipher.final()])),
      (tag = cipher.getAuthTag()),
      (encryptedDerivationPath = Buffer.concat([encrypted, tag]).toString("hex")));
  } catch (error) {
    throw `Could not encrypt the serialized derivation path '${error}'`;
  }
  ...
  try {
    ...
    throw `Could not convert addressRaw into base58 format '${error}'`;
  }
...
function getByronDaedalusAddressPath(rootPubKeyHex, address) {
  ...
  try {
    ...
    throw `Could not convert address from base58 to bytes '${error}'`;
  }
  ...
  try {
    ...
    throw `Could not cbor decode the address bytes '${error}'`;
  }
  if (addressDataTagged.tag != 24 || !Buffer.isBuffer(addressDataTagged.value)) {
    throw `Tag in addressData is not 24 or addressData value is not a buffer'`;
  }
  ...
  try {
    ...
    throw `Could not cbor decode the addressData in the address '${error}'`;
  }
  ...
  try {
    ...
    throw `Could not cbor decode the encryptedDerivationPath data '${error}'`;
  }
  ...
  try {
    ((decipher = crypto.createDecipheriv("chacha20-poly1305", key, nonce, {
      ...
      (decryptedDerivationPath = Buffer.concat([decipher.update(data), decipher.final()])));
  } catch (error) {
    throw `Could not decrypt the serialized derivation path '${error}'`;
  }
  ...
  try {
    ...
    throw `Could not cbor decode the decryptedDerivationPath '${error}'`;
  }
...
function readByronDaedalusAddress(addr) {
  ...
  // first check, if the given address is an empty string, exit with an error
  if (trimString(addr) == "") {
    throw `Error: The address value is empty`;
  }
  ...
  try {
    ...
    try {
      // inner try to check if the content is a base58 address
      var tmp = CardanoWasm.ByronAddress.from_base58(content);
      var addr_base58 = content;
    } catch (error) {
      throw `The address in file '${addr}' is not a valid base58 byron daedalus address`;
    }
  } catch (error) {}
  ...
  if (addr_base58 == "") {
    ...
    //check that the given key is a hex string
    if (!regExpHex.test(addr)) {
      throw `Provided address '${addr}' is not a valid base58 address, file or hex-string(addressRaw), or the file is missing`;
    }
    try {
      var addr_base58 = CardanoWasm.ByronAddress.from_bytes(Buffer.from(addr, "hex")).to_base58();
    } catch (error) {
      throw `Provided addressRaw hex string is not a valid byron daedalus address format '${error}'`;
    }
  }
...
function unscrambleDaedalusPaperWalletMnemonic(mnemonic, password = "") {
  ...
  if (wordCount(mnemonic) != 27) {
    throw `Please provide a 27 word daedalus byron paper-wallet mnemonic!`;
  }
  ...
  const passphraseHex = crypto
    .pbkdf2Sync(Buffer.from(passphraseMnemonic, "utf8"), Buffer.from(salt, "utf8"), 2048, 32, "sha512")
    .toString("hex");
  ...
  try {
    ...
    throw `The scrambled mnemonic part (first 18 words) is not a valid one '${error}'`;
  }
  if (saltLen >= scrambledEntropy.length) {
    throw "scrambled entropy is too short, must be larger then ${saltLen} bytes.";
  }
  ...
  outBuffer.forEach((element, index) => {
    outBuffer[index] = outBuffer[index] ^ scrambledEntropy[saltLen + index];
  });
...
function unscrambleYoroiPaperWalletMnemonic(mnemonic, password = "") {
  ...
  if (wordCount(mnemonic) != 21) {
    throw `Please provide a 21 word yoroi byron paper-wallet mnemonic!`;
  }
  ...
  try {
    ...
    throw `The provided 21 words are not valid ones '${error}'`;
  }
  ...
  try {
    ...
    throw `Could not unscramble via cardano-wallet lib '${error}'`;
  }
...
function deriveBip32Child(prvKeyHex, index) {
  ...
  if (index >= 0x80000000) {
    // HARDENED -> derive from the private key
    var data = new Uint8Array([0x00, ...prvKeyBuffer, ...indexBuffer]);
  } else {
    // SOFT -> derive from the public key
    try {
      const pubKeyBuffer = secp256k1.publicKeyCreate(prvKeyBuffer);
      var data = new Uint8Array([...pubKeyBuffer, ...indexBuffer]);
    } catch (error) {
      throw `Could not get PublicKey for soft-derivation in Bip32Child mode '${error}'`;
    }
  }
...
// function to convert a Map into JSON
const mapToJs = (obj) => {
...
function verifyCIP8(workMode = "verify-cip8", calling_args = process.argv.slice(3)) {
  //default calling arguments are the same as calling the main process - can be modified by subfunctions
  ...
  if (typeof data_hex === "string") {
    // a nullstring is also ok
    ...
    if (payload_data_hex != "" && !regExpHex.test(payload_data_hex)) {
      throw { msg: `Data is not a valid hex string` };
    }
  }

  if (!payload_data_hex) {
    //no payload_data_hex present, lets try the data-file parameter
    ...
    if (typeof payload_data_file === "string" && payload_data_file != "") {
      ...
      try {
        ...
        throw { msg: `Can't read data-file '${payload_data_file}'` };
      }
    }
  }

  if (!payload_data_hex) {
    //no payload_data_hex present, lets try the data (data-hex) parameter
  ...
  if (typeof COSE_Key_cbor_hex === "undefined" || COSE_Key_cbor_hex === true) {
    throw { msg: `Missing COSE_Key parameter --cose-key`, showUsage: workMode };
  }
  ...
  if (!regExpHex.test(COSE_Key_cbor_hex)) {
    throw { msg: `COSE_Key is not a valid hex string` };
  }
  ...
  try {
    ...
    throw { msg: `Can't cbor decode the given COSE_Key signature (${error})` };
  }

  //do a sanity check on the decoded COSE_Key_structure
  if ((!COSE_Key_structure) instanceof Map || COSE_Key_structure.size < 4) {
    throw { msg: `COSE_Key is not valid. It must be a map with at least 4 entries: kty,alg,crv,x.` };
  } else if (COSE_Key_structure.get(1) != 1) {
    throw { msg: `COSE_Key map key '1' (kty) is not '1' (OKP)` };
  } else if (COSE_Key_structure.get(3) != -8) {
    throw { msg: `COSE_Key map key '3' (alg) is not '-8' (EdDSA)` };
  } else if (COSE_Key_structure.get(-1) != 6) {
    throw { msg: `COSE_Key map key '-1' (crv) is not '6' (Ed25519)` };
  } else if (!COSE_Key_structure.has(-2)) {
    throw { msg: `COSE_Key map key '-2' (public key) is missing` };
  }

  //get the publickey
  var pubKey_buffer = COSE_Key_structure.get(-2);
  if (!Buffer.isBuffer(pubKey_buffer)) {
    throw { msg: `PublicKey entry in the COSE_Key is not a bytearray` };
  }
  ...
  if (typeof COSE_Sign1_cbor_hex === "undefined" || COSE_Sign1_cbor_hex === true) {
    throw { msg: `Missing COSE_Sign1 signature parameter --cose-sign1`, showUsage: workMode };
  }
  ...
  if (!regExpHex.test(COSE_Sign1_cbor_hex)) {
    throw { msg: `COSE_Sign1 is not a valid hex string` };
  }
  ...
  try {
    ...
    throw { msg: `Can't cbor decode the given COSE_Sign1 signature (${error})` };
  }

  //do a sanity check on the decoded COSE_Sign1_structure
  if (COSE_Sign1_structure instanceof Array == false || COSE_Sign1_structure.length != 4) {
    throw { msg: `COSE_Sign1 is not a valid signature. It must be an array with 4 entries.` };
  }
  ...
  if (!Buffer.isBuffer(protectedHeader_buffer)) {
    throw { msg: `Protected header is not a bytearray (serialized) cbor` };
  }
  //cbor decode the protectedHeader_buffe into protectedHeader Map
  try {
    var protectedHeader = cbor.decode(protectedHeader_buffer);
  } catch (error) {
    throw { msg: `Can't cbor decode the protected header (${error})` };
  }

  //extract the content and do a check on the map entries
  if (!protectedHeader.has(1)) {
    throw { msg: `Protected header map key '1' is missing` };
  } else if (protectedHeader.get(1) != -8) {
    throw { msg: `Protected header map key '1' (alg) is not '-8' (EdDSA)` };
  } else if (!protectedHeader.has("address")) {
    throw { msg: `Protected header map key 'address' is missing` };
  }
  var sign_addr_buffer = protectedHeader.get("address");
  if (!Buffer.isBuffer(sign_addr_buffer)) {
    throw { msg: `Protected header map value of key 'address' is not a bytearray` };
  }
  ...
  if (typeof address === "string") {
    //do the check if the parameter is provided
    ...
    if (sub_args["nohashcheck"] === false && !sign_addr.matchPubKey) {
      //exit with an error if the address does not contain the pubKey hash
      throw {
        msg: `The given ${sign_addr.type} address '${sign_addr.addr}' does not belong to the public key in the COSE_Key.`,
      };
    }
    ...
    if (protectedHeader.has(4)) {
      protectedHeader.set(4, Buffer.from(sign_addr.hex, "hex"));
    }

    //convert the modified protectedHeader Map back into cbor hex
    ...
    if (sub_args["nohashcheck"] === false && !sign_addr.matchPubKey) {
      //exit with an error if the address does not contain the pubKey hash
      throw {
        msg: `The ${sign_addr.type} address '${sign_addr.addr}' in the COSE_Sign1 does not belong to the public key in the COSE_Key.`,
      };
    }

    //no modifications to the protected header, reuse the existing buffer
    var protectedHeader_cbor_hex = Buffer.from(protectedHeader_buffer).toString("hex");
  }
  ...
  if (unprotectedHeader instanceof Map == false && typeof unprotectedHeader === "object") {
    // so if it is not a map but an object, convert it
    var unprotectedHeader = new Map(Object.entries(unprotectedHeader));
  }

  if (unprotectedHeader instanceof Map == false) {
    // if its not a map now, throw an error
    throw { msg: `Unprotected header is not a map` };
  }

  if (!unprotectedHeader.has("hashed")) {
    throw { msg: `Unprotected header key 'hashed' is missing` };
  }
  var isHashed = unprotectedHeader.get("hashed");
  if (typeof isHashed !== "boolean") {
    throw { msg: `Unprotected header value in key 'hashed' is not a boolean` };
  }
  ...
  if (payload_data_hex && isHashed) {
    payload_data_hex = getHash(payload_data_hex, 28);
  } //hash the payload with blake2b_224 (28bytes digest length) }
  ...
  if (typeof payload_data_hex === "undefined") {
    ...
    if (Buffer.isBuffer(payload_data_buffer)) {
      // payload present, load it into payload_data_hex
      var payload_data_hex = payload_data_buffer.toString("hex");
    } else if (payload_data_buffer == null) {
      // payload is missing, and there is also no payload provided via the optional data parameters
      throw {
        msg: `There is no payload present in the COSE_Sign1 signature, please provide a payload via the data / data-hex / data-file parameters`,
      };
    }
  }
  ...
  if (!Buffer.isBuffer(signature_buffer)) {
    throw { msg: `Signature is not a bytearray` };
  }
  ...
  var Sig_structure = [
    ...
    Buffer.from(""),
    Buffer.from(payload_data_hex, "hex"),
  ];
  ...
  if (sub_args["json"] === true) {
    //generate content in json format
    var content = `{ "result": "${verified}" }`;
  } else if (sub_args["json-extended"] === true) {
    //generate content in json format with additional fields
    ...
    if (payload_data_hex.length <= 2000000) {
      content += `"payloadDataHex": "${payload_data_hex}", `;
    } //only include the payload_data_hex if it is less than 2M of chars
    content += `"isHashed": "${isHashed}",`;
    if (Sig_structure_cbor_hex.length <= 2000000) {
      content += `"verifyDataHex": "${Sig_structure_cbor_hex}", `;
    } //only include the Sig_structure_cbor_hex if it is less than 2M of chars
    content += `"signature": "${signature_hex}", "publicKey": "${pubKey}"`;
    if (sub_args["include-maps"] === true) {
      //generate content also with JSON-Maps for the COSE_Key, COSE_Sign1 and verifyData structures
    ...
    //generate content in text format
...
function signCIP8(workMode = "sign-cip8", calling_args = process.argv.slice(3)) {
  //default calling arguments are the same as calling the main process - can be modified by subfunctions
  ...
  if (typeof key_file_hex === "undefined" || key_file_hex === true) {
    throw { msg: `Missing secret key parameter`, showUsage: workMode };
  }
  ...
  try {
    if (sign_key.length <= 64) {
      var prvKey = CardanoWasm.PrivateKey.from_normal_bytes(Buffer.from(sign_key, "hex"));
    } else {
      var prvKey = CardanoWasm.PrivateKey.from_extended_bytes(Buffer.from(sign_key.substring(0, 128), "hex"));
    } //use only the first 64 bytes (128 chars)
  } catch (error) {
    throw { msg: `Could not load the signing/secret key - ${error}` };
  }
  ...
  var address = sub_args["address"];
  if (typeof address === "undefined" || address === true) {
    throw { msg: `Missing signing address parameter`, showUsage: workMode };
  }
  ...
  if (sign_addr.network == "mainnet" && !(typeof sub_args["testnet-magic"] === "undefined")) {
    // check for mainnet address
    throw {
      msg: `The mainnet ${sign_addr.type} address '${sign_addr.addr}' does not match your current '--testnet-magic xxx' setting.`,
    };
  } else if (sign_addr.network == "testnet" && typeof sub_args["testnet-magic"] === "undefined") {
    // check for testnet address
    throw {
      msg: `The testnet ${sign_addr.type} address '${sign_addr.addr}' does not match your current setting. Use '--testnet-magic xxx' for testnets.`,
    };
  }
  ...
  if (sub_args["nohashcheck"] === false && !sign_addr.matchPubKey) {
    //exit with an error if the address does not contain the pubKey hash
    throw { msg: `The ${sign_addr.type} address '${sign_addr.addr}' does not belong to the provided secret key.` };
    process.exit(1);
  }
  ...
  if (typeof payload_data_hex === "undefined" || payload_data_hex === true) {
    ...
    if (typeof payload_data === "undefined" || payload_data === true) {
      ...
      var payload_data_file = sub_args["data-file"];
      if (typeof payload_data_file === "undefined" || payload_data_file === true) {
        throw { msg: `Missing data / data-hex / data-file to sign`, showUsage: workMode };
      }
    ...
    if (payload_data_hex != "" && !regExpHex.test(payload_data_hex)) {
      throw { msg: `Data to sign is not a valid hex string` };
    }
  }
  ...
  // kid (4) - Optional, if present must be set to the same value as in the COSE_Key specified below. It is recommended to be set to the same value as in the "address" header.
  // "address" - must be set to the raw binary bytes of the address as per the binary spec, without the CBOR binary wrapper tag
  var protectedHeader_cbor_hex = Buffer.from(
    cbor.encode(new Map().set(1, -8).set("address", Buffer.from(sign_addr.hex, "hex"))),
  ).toString("hex");
  ...
  //  bstr                ; signature
  //  ]
  if (sub_args["nopayload"]) {
    //the payload can be excluded from the COSE_Sign1 signature if the payload is known by the involved entities
    var COSE_Sign1_structure = [
      Buffer.from(protectedHeader_cbor_hex, "hex"),
      unprotectedHeader,
      null,
  ...
  if (sub_args["json"] === true) {
    ...
    const signedMsg_checksum = base64url.encode(
      Buffer.from(
        new Uint8Array(new Uint32Array([fnv32.fnv_1a(Buffer.from(COSE_Sign1_cbor_hex, "hex"))]).buffer).reverse(),
      ),
    );
    ...
    if (payload_data_hex_orig.length <= 2000000) {
      content += `"inputDataHex": "${payload_data_hex_orig}", `;
    } //only include the payload_data_hex if it is less than 2M of chars
    content += `"isHashed": "${isHashed}",`;
    if (isHashed) {
      content += `"hashedInputDataHex": "${payload_data_hex}", `;
    } //only include the payload_data_hex(now in hashed format) if isHashed is true
    if (Sig_structure_cbor_hex.length <= 2000000) {
      content += `"signDataHex": "${Sig_structure_cbor_hex}", `;
    } //only include the Sig_structure_cbor_hex if it is less than 2M of chars
    content += `"signature": "${signature_hex}", "publicKey": "${pubKey}",`;
    if (sub_args["include-secret"] === true) {
      //generate content also with the secret key
    ...
    if (sub_args["signature-only"] === false) {
      content += ` ${COSE_Key_cbor_hex}`;
    } //the output of the COSE_Key can be surpressed via the --signature-only flag
  }
...
async function main() {
  ...
  if (
    ...
    process.argv[2].toLowerCase().includes("usage")
  ) {
  ...
  //show version
  if (process.argv[2].toLowerCase().includes("version")) {
    console.log(`${appname} ${version}`);
    process.exit(0);
  }

  //first parameter - workMode: "sign, verify or keygen"
  ...
  workmodeFlags.forEach((element) => {
    args[element] ? (workMode += `-${element}`) : {};
  });
  ...
  switch (workMode) {
    case "sign": //SIGN DATA IN DEFAULT MODE
      ...
      if (typeof sign_data_hex === "undefined" || sign_data_hex === true) {
        ...
        if (typeof sign_data === "undefined" || sign_data === true) {
          ...
          if (typeof sign_data_file === "undefined" || sign_data_file === true) {
            console.error(`Error: Missing data / data-hex / data-file to sign`);
            showUsage(workMode);
          }
          ...
          try {
            ...
            console.error(`Error: Can't read data-file '${sign_data_file}'`);
            process.exit(1);
          }
        } else {
      ...
      if (sign_data_hex != "" && !regExpHex.test(sign_data_hex)) {
        console.error(`Error: Data to sign is not a valid hex string`);
        process.exit(1);
      }
      ...
      if (typeof key_file_hex === "undefined" || key_file_hex === true) {
        console.error(`Error: Missing secret key parameter`);
        showUsage(workMode);
      }
      ...
      try {
        if (sign_key.length <= 64) {
          ...
          var prvKey = CardanoWasm.PrivateKey.from_extended_bytes(Buffer.from(sign_key.substring(0, 128), "hex"));
        } //use only the first 64 bytes (128 chars), rest is publicKey + chainCode
      } catch (error) {
      ...
      if (typeof address === "string") {
        ...
        if (!sign_addr.matchPubKey) {
          //exit with an error if the address does not contain the pubKey hash
          console.error(
            `Error: The ${sign_addr.type} address '${sign_addr.addr}' does not belong to the provided secret key.`,
          );
          process.exit(1);
        }
      }
      ...
      if (args["jcli"] === true) {
        //convert signature and publickey in bech format
        try {
          ...
          console.error(`${error}\nCouldn't encode signature/pubKey into bech string.`);
          process.exit(1);
        }
      }
      ...
      if (args["json"] === true) {
      ...
      } else if (args["json-extended"] === true) {
        //generate content in json format with additional fields
        var prvKeyHex = Buffer.from(prvKey.as_bytes()).toString("hex");
        var content = `{ "workMode": "${workMode}", `;
        if (sign_data_hex.length <= 2000000) {
          content += `"signDataHex": "${sign_data_hex}", `;
        } //only include the sign_data_hex if it is less than 2M of chars
        if (sign_addr) {
          content += `"addressHex": "${sign_addr.hex}", "addressType": "${sign_addr.type}", "addressNetwork": "${sign_addr.network}", `;
        } //only include the signing address if provided
        content += `"signature": "${signature}", "publicKey": "${pubKey}"`;
        if (args["include-secret"] === true) {
          //generate content also with the secret key
          content += `, "secretKey": "${prvKeyHex}"`;
        }
        content += ` }`;
        ...
        if (args["signature-only"] === false) {
          content += ` ${pubKey}`;
        } //the output of the public-key can be surpressed via the --signature-only flag
      }
      ...
      if (typeof out_file === "undefined" || out_file === true) {
        console.log(content);
      } //Output to console
      else {
        //else try to write the content out to the given file
        try {
    ...
    case "sign-cip36": //SIGN REGISTRATION DATA IN CIP-36 MODE (Catalyst)
      ...
      if (typeof address === "undefined" || address === true) {
        console.error(`Error: Missing rewards payout address (--payment-address)`);
        showUsage(workMode);
      }
      ...
      if (!rewards_addr.type.includes("payment")) {
        console.error(
          `Error: The rewards address '${rewards_addr.addr}' is not a payment address starting with 'addr...'`,
        );
        process.exit(1);
      }

      //check that the given address belongs to the current network
      if (rewards_addr.network == "mainnet" && !(typeof args["testnet-magic"] === "undefined")) {
        // check for mainnet address
        console.error(
          `Error: The mainnet ${rewards_addr.type} address '${rewards_addr.addr}' does not match your current '--testnet-magic xxx' setting.`,
        );
        process.exit(1);
      } else if (rewards_addr.network == "testnet" && typeof args["testnet-magic"] === "undefined") {
        // check for testnet address
        console.error(
          `Error: The testnet ${rewards_addr.type} address '${rewards_addr.addr}' does not match your current setting. Use '--testnet-magic xxx' for testnets.`,
        );
        process.exit(1);
      }
      ...
      if (typeof vote_public_key === "undefined") {
        console.error(
          `Error: Missing vote public key(s) parameter. For a deregistration please use the flag --deregister.`,
        );
        showUsage(workMode);
      }

      //if there is only one --vote-public-key parameter present, convert it to an array
      if (typeof vote_public_key === "string") {
        vote_public_key = [vote_public_key];
      }

      //get deleg voting weight -> store it in vote_weight
      var vote_weight = args["vote-weight"];
      if (typeof vote_weight === "undefined") {
        vote_weight = 1;
      }
      if (vote_weight === true) {
        console.error(`Error: Please specify a --vote-weight parameter with an unsigned integer value >= 0`);
        process.exit(1);
      }

      //if there is only one --vote-weight parameter present, convert it to an array
      if (typeof vote_weight === "number") {
        vote_weight = [vote_weight];
      }

      //if not the same amounts of vote_public_keys and vote_weights provided, show an error
      if (vote_public_key.length != vote_weight.length) {
        console.error(
          `Error: Not the same count of --vote-public-key(` +
            vote_public_key.length +
            `) and --vote-weight(` +
            vote_weight.length +
            `) parameters`,
        );
      ...
      for (let cnt = 0; cnt < vote_public_key.length; cnt++) {
        ...
        if (typeof entry_vote_public_key === "number" || entry_vote_public_key === true) {
          console.error(`Error: Invalid --vote-public-key parameter found, please use a filename, hex- or bech-string`);
          process.exit(1);
        }
        entry_vote_public_key_hex = readKey2hex(entry_vote_public_key, "public");
        entry_vote_weight = vote_weight[cnt];
        if (typeof entry_vote_weight !== "number" || entry_vote_weight < 0) {
          console.error(`Error: Please specify a --vote-weight parameter with an unsigned integer value >= 0`);
          process.exit(1);
        }
      ...
      if (all_vote_keys_array.indexOf(pubKey) > -1) {
        console.error(
          `Error: Wrong vote-public-key entry found, or your secret-key is a wrong one. The vote-public-key(s) must be different from the public-key of the signing stake-secret-key.`,
        );
        process.exit(1);
      }
      ...
      if (total_vote_weight == 0) {
        console.error(
          `Error: Total vote-weight is zero, please make sure that at least one vote-public-key has a vote-weight > 0`,
        );
        process.exit(1);
      }
      ...
      if (typeof nonce === "undefined") {
        var totalUtcSeconds = Math.floor(new Date().getTime() / 1000);
        nonce = 4492800 + (totalUtcSeconds - 1596059091);
      } //if not defined, set it to the slotHeight of cardano-mainnet
      else if (typeof nonce !== "number" || nonce === true || nonce < 0) {
        console.error(
          `Error: Please specify a --nonce parameter with an unsigned integer value > 0, or remove the parameter so the mainnet slotHeight will be calculated from current time`,
        );
        process.exit(1);
      }
      ...
      if (typeof vote_purpose_param === "undefined") {
        vote_purpose = 0;
      } //if not defined, set it to default=0
      else if (typeof vote_purpose_param === "number" && vote_purpose_param >= 0) {
        vote_purpose = vote_purpose_param;
      } else {
        console.error(`Error: Please specify a --vote-purpose parameter with an unsigned integer value >= 0`);
        process.exit(1);
      }
      ...
      switch (vote_purpose) {
        case 0:
          var vote_purpose_description = "Catalyst";
          break;
        default:
          var vote_purpose_description = "Unknown";
      }
      ...
      /*
			...
			*/
      const delegationMap = new Map().set(
        61284,
        new Map()
          .set(1, vote_delegation_array)
          .set(2, Buffer.from(pubKey, "hex"))
          .set(3, Buffer.from(rewards_addr.hex, "hex"))
          .set(4, nonce)
          .set(5, vote_purpose),
      );
    ...
    case "sign-cip36-deregister": //SIGN DEREGISTRATION DATA IN CIP-36 MODE (Catalyst)
      ...
      /*
			...
			*/
      const dereg_Map = new Map().set(
        61286,
        new Map().set(1, Buffer.from(pubKey, "hex")).set(2, nonce).set(3, vote_purpose),
      );
    ...
    case "sign-cip88": //SIGN REGISTRATION DATA IN CIP-86 MODE (Pool-ID-Registration)
      ...
      if (typeof key_file_hex === "undefined" || key_file_hex === true) {
        console.error(`Error: Missing calidus public key parameter`);
        showUsage(workMode);
      }
      ...
      //load the private key (normal or extended)
      try {
        if (prvKeyHex.length <= 64) {
          var prvKey = CardanoWasm.PrivateKey.from_normal_bytes(Buffer.from(prvKeyHex, "hex"));
        } else {
          var prvKey = CardanoWasm.PrivateKey.from_extended_bytes(Buffer.from(prvKeyHex.substring(0, 128), "hex"));
        } //use only the first 64 bytes (128 chars)
      } catch (error) {
        console.error(`Error: ${error}`);
        process.exit(1);
      ...
      var payloadMap = new Map()
        .set(1, [1, Buffer.from(poolIdHex, "hex")])
        .set(2, [])
        .set(3, [2])
        .set(4, nonce)
        .set(7, Buffer.from(calidusPubKeyHex, "hex"));
    ...
    case "verify": //VERIFY DATA IN DEFAULT MODE
      ...
      if (typeof verify_data_hex === "undefined" || verify_data_hex === true) {
        ...
        if (typeof verify_data === "undefined" || verify_data === true) {
          ...
          if (typeof verify_data_file === "undefined" || verify_data_file === true) {
            console.error(`Error: Missing data / data-hex / data-file to verify`);
            showUsage(workMode);
          }
          ...
          try {
            ...
            console.error(`Error: Can't read data-file '${verify_data_file}'`);
            process.exit(1);
          }
        } else {
      ...
      if (!regExpHex.test(verify_data_hex)) {
        console.error(`Error: Data to verify is not a valid hex string`);
        process.exit(1);
      }

      //get the signature to verify -> store it in signature
      var signature = args["signature"];
      if (typeof signature === "undefined" || signature === true) {
      ...
      if (!regExpHex.test(signature)) {
        try {
          ...
          console.error(`Error: Signature is not a valid hex string or a valid bech encoded signature`);
          process.exit(1);
        }
      }
      ...
      if (typeof key_file_hex === "undefined" || key_file_hex === true) {
        console.error(`Error: Missing public key parameter`);
        showUsage(workMode);
      }
      ...
      if (typeof address === "string") {
        ...
        if (!verify_addr.matchPubKey) {
          //exit with an error if the address does not contain the pubKey hash
          console.error(
            `Error: The ${verify_addr.type} address '${verify_addr.addr}' does not belong to the provided public key.`,
          );
          process.exit(1);
        }
      }
      ...
      if (args["json"] === true) {
        ...
        if (verify_data_hex.length <= 2000000) {
          content += `"verifyDataHex": "${verify_data_hex}", `;
        } //only include the verify_data_hex if it is less than 2M of chars
        if (typeof verify_addr !== "undefined") {
          content += `"addressHex": "${verify_addr.hex}", "addressType": "${verify_addr.type}", "addressNetwork": "${verify_addr.network}", `;
        } //only include the verification address if provided
    ...
    case "keygen-exodus-stake": // exodus style 12 words (stake address)
      ...
      var XpubKeyHex = "",
        ...
        drepIdHex = "",
        drepIdBech = "";
      ((drepIdHexLegacy = ""), (drepIdBechLegacy = ""));
      var ccColdIdHex = "",
        ccColdIdBech = "",
        ccHotIdHex = "",
        ccHotIdBech = "";
      ...
      var rootKeyHex = "",
        ...
        rootPubKeyHex = "",
        calidusIdHex = "",
        outIdBech = ""; //option to write out the generated id to a file
      ...
      if (typeof derivation_path === "string" && derivation_path != "") {
        //ok, a path was provided let check
        ...
        switch (derivation_path) {
          ...
          case "STAKE":
            ...
            break;
          case "CIP36":
            derivation_path = "1694H/1815H/0H/0/0";
            break;
          case "DREP":
          ...
          case "CC-HOT":
            ...
            break;
          case "POOL":
            derivation_path = "1853H/1815H/0H/0H";
            break;
          case "CALIDUS":
            derivation_path = "1852H/1815H/0H/0/0";
            break;
        ...
        if (derivation_path.indexOf(`'`) > -1) {
          derivation_path = derivation_path.replace(/'/g, "H");
        } //replace the ' char with a H char
        switch (workMode) {
          case "keygen-byron": //check against byron daedalus path regex
            if (!regExpPathByron.test(derivation_path)) {
              console.error(
                `Error: The provided derivation --path '${derivation_path}' for byron does not match the right format! Example: 0H/0H`,
              );
              process.exit(1);
            }
            break;
          case "keygen-yoroi": //check against byron yoroi path regex
            if (!regExpPathYoroi.test(derivation_path)) {
              console.error(
                `Error: The provided derivation --path '${derivation_path}' for byron yoroi does not match the right format! Must always start with 44H/1815H/...`,
              );
              process.exit(1);
            }
            break;
          ...
          case "keygen-exodus-stake":
            if (!regExpPathExodus.test(derivation_path)) {
              console.error(
                `Error: The provided derivation --path '${derivation_path}' for exodus does not match the needed format 44H/1815H/...`,
              );
              process.exit(1);
            }
            break;
          default: //check against normal path regex
            if (!regExpPath.test(derivation_path)) {
              console.error(
                `Error: The provided derivation --path '${derivation_path}' does not match the right format! Example: 1852H/1815H/0H/0/0`,
              );
              process.exit(1);
            }
            break;
      ...
      if (args["cip36"] === true) {
        var derivation_path = "1694H/1815H/0H/0/0";
      }
      ...
      if (typeof mnemonics === "string" && mnemonics != "") {
        //ok, mnemonics were provided let check
        ...
        try {
          ...
          if (mnemonicsWordCount < 12 || mnemonicsWordCount > 27) {
            console.error(`Error: The given mnemonics-file does not contain between 12 and 27 words.`);
            process.exit(1);
          }
        } catch (error) {
          ...
          var mnemonicsWordCount = wordCount(mnemonics);
          if (mnemonicsWordCount < 12 || mnemonicsWordCount > 27) {
            console.error(`Error: Please provide between 12 and 27 words for the --mnemonics.`);
            process.exit(1);
          }
        }
        ...
        if (mnemonicsWordCount == 27) {
          try {
            ...
            if (mnemonicsWordCount != 12) {
              throw `Unscrambled Mnemonics are ${mnemonicsWordCount} words and not 12 words!?`;
            }
          } catch (error) {
            console.error(`Error: Could not unscrable the given 27 word paper-wallet mnemonics. '${error}'`);
            process.exit(1);
          }
          ...
          try {
            ...
            if (mnemonicsWordCount != 15) {
              throw `Unscrambled Mnemonics are ${mnemonicsWordCount} words and not 15 words!?`;
            }
            var entropy = Buffer.from(bip39.mnemonicToEntropy(mnemonics), "hex");
          } catch (error) {
            console.error(`Error: Could not unscrable the given 21 word yoroi paper-wallet mnemonics. '${error}'`);
            process.exit(1);
          }
        } else {
          ...
          try {
            ...
            console.error(`Error: The provided mnemonics are not valid, please check the correct spelling. '${error}'`);
            process.exit(1);
          }
        }
        ...
        if (derivation_path == "") {
          switch (workMode) {
            case "keygen-byron": //byron daedalus default
              derivation_path = "0H/0H";
              break;
            case "keygen-yoroi": //byron yoroi default
              derivation_path = "44H/1815H/0H/0/0";
              break;
        ...
        //no mnemonics provided, generate a random entropy and get the mnemonics from it. for byron and yoroi, set the default derivation path
        ...
        switch (workMode) {
          ...
          case "keygen-trezor":
            ...
            break; //new random entropy 256 bits -> 24 words
            break;
      ...
      if (derivation_path == "") {
        //generate a simple ed25519 keypair
        ...
        try {
          ...
          console.error(`Error: Could not generate a new ed25519 keypair. '${error}'`);
          process.exit(1);
        }
        var entropy = "",
          mnemonics = "";
        ...
        //derivation path is present, so we derive the rootKey via mnemonics
        ...
        switch (workMode) {
          case "keygen-ledger": // generate a rootkey via ledger derivation method
            try {
              ...
              console.error(`Error: Could not generate the rootKey for ledger type mnemonics. '${error}'`);
              process.exit(1);
            }
          ...
          case "keygen-trezor": // generate a rootkey via trezor derivation method
            switch (mnemonicsWordCount) {
              ...
              case 18: //for 12,15 or 18 words the derivation type of trezor is the normal icarus
                try {
                  var rootKey = CardanoWasm.Bip32PrivateKey.from_bytes(
                    (masterKey = generateIcarusMasterKey(entropy, passphrase)),
                  );
                } catch (error) {
                  console.error(
                    `Error: Could not generate the rootKey for trezor type 12/15/18words mnemonic. '${error}'`,
                  );
                  process.exit(1);
                }
              ...
              case 24: //for 24 words we have to deal with the trezor-bug first
                try {
                  ...
                  var rootKey = CardanoWasm.Bip32PrivateKey.from_bytes(
                    (masterKey = generateIcarusMasterKey(newentropy, passphrase)),
                  );
                } catch (error) {
                  console.error(`Error: Could not generate the rootKey for trezor type 24words mnemonic. '${error}'`);
                  process.exit(1);
                }
              ...
              default: // there are only 12,15,18 or 24 words allowed with trezor, throw an error otherwise
                ...
                console.error(
                  `Error: Could not generate the rootKey for icarus/normal type from the entropy/mnemonic. '${error}'`,
                );
                process.exit(1);
                break;
            }
          ...
          case "keygen-byron": // generate a rootkey via byron daedalus derivation method
            switch (mnemonicsWordCount) {
              case 12: //for 12 words DaedalusStyle
                try {
                  var rootKeyByron = CardanoWasm.Bip32PrivateKey.from_bytes(
                    (masterKey = generateByronMasterKeyFromMnemonic(mnemonics)),
                  );
                  ...
                  console.error(
                    `Error: Could not generate the rootKey for byron daedalus style 12 words mnemonics. '${error}'`,
                  );
                  process.exit(1);
                }
                ...
                if (typeof address === "string") {
                  try {
                    ...
                    console.error(`Error: Could not get derivation path from byron-address '${error}'`);
                    process.exit(1);
                  }
                }
                break;
              ...
              default: // there is only support for 12 words currently
                ...
                console.error(
                  `Error: Could not generate the rootKey for byron-daedalus type from the entropy/mnemonic. '${error}'`,
                );
                process.exit(1);
                break;
            }
          ...
          case "keygen-yoroi": // generate a rootkey via byron yoroi derivation method
            switch (mnemonicsWordCount) {
              case 15: //for 15 words Yoroi-Style
                try {
                  if (paperWalletMnemonics == "") {
                    //in case the given mnemonic was generated from a 21 word yoroi paper wallet, don't use the passphrase again
                    var rootKey = CardanoWasm.Bip32PrivateKey.from_bytes(
                      (masterKey = generateIcarusMasterKey(entropy, passphrase)),
                    );
                  } else {
                    var rootKey = CardanoWasm.Bip32PrivateKey.from_bytes(
                      (masterKey = generateIcarusMasterKey(entropy)),
                    );
                  }
                } catch (error) {
                  console.error(
                    `Error: Could not generate the rootKey for byron yoroi style 15 words mnemonic. '${error}'`,
                  );
                  process.exit(1);
                }
                derivation_type = "icarus";
              ...
              default: // there is only support for 15 words currently
                ...
                console.error(
                  `Error: Could not generate the rootKey for byron-yoroi type from the entropy/mnemonic. '${error}'`,
                );
                process.exit(1);
                break;
            }
            ...
            switch (mnemonicsWordCount) {
              case 12: //for 12 words ExodusStyle
                try {
                  ...
                  console.error(
                    `Error: Could not generate the rootKey for exodus style 12 words mnemonics. '${error}'`,
                  );
                  process.exit(1);
                }
              ...
              default: // there is only support for 12 words currently
                ...
                console.error(
                  `Error: Could not generate the rootKey for exodus type from the entropy/mnemonic. '${error}'`,
                );
                process.exit(1);
                break;
            }
        ...
        //store the rootKey in hex format for output later on
        if (rootKeyHex == "") {
          rootKeyHex = Buffer.from(rootKey.as_bytes()).toString("hex");
        }
          ...
          switch (workMode) {
            case "keygen-byron": //in byron daedalus mode, use the deriveByronDaedalus function and work with the hex string
              try {
                ...
                console.error(`Error: Could not derive the given path from the rootKey in byron mode. '${error}'`);
                process.exit(1);
              }
              break;
            ...
            case "keygen-exodus-stake":
              try {
                ...
                console.error(`Error: Could not derive the given path from the rootKey in exodus mode. '${error}'`);
                process.exit(1);
              }
              break;
            default: //for all other modes, derive via the .derive function of the already created rootKey
              try {
                ...
                console.error(`Error: Could not derive the given path from the rootKey. '${error}'`);
                process.exit(1);
              }
        ...
        switch (workMode) {
          case "keygen-byron":
            try {
              var rootKey = CardanoWasm.PrivateKey.from_extended_bytes(
                Buffer.from(byronKeyHex.substring(0, 128), "hex"),
              );
            } catch (error) {
              console.error(`Error: Could not load generated byron-private key ${error}`);
              process.exit(1);
            }
            ...
            try {
              ...
              console.error(`Error: Could not generated byron-address '${error}'`);
              process.exit(1);
            }
            break;
          ...
          case "keygen-exodus-stake":
            ...
            var exodusKeyHex = Buffer.from(generateByronMasterKeyFromSeed(exodusKeyHex.substring(0, 64), 1)).toString(
              "hex",
            );

            try {
              var rootKey = CardanoWasm.PrivateKey.from_extended_bytes(
                Buffer.from(exodusKeyHex.substring(0, 128), "hex"),
              );
            } catch (error) {
              console.error(`Error: Could not load generated exodus-private key ${error}`);
              process.exit(1);
            }
            ...
            switch (workMode) {
              case "keygen-exodus": //generate the exodus-address base-address -> they use the same private/public key for payment and stake part, so also the same public key (hash) twice
                try {
                  ...
                  console.error(`Error: Could not generated exodus base-address '${error}'`);
                  process.exit(1);
                }
                break;
      ...
      switch (derivation_path.substring(0, 11)) {
        ...
        case "1853H/1815H": //pool keys
          //generate the secret key json and bech format
          ...
          var prvKeyBech = bech32.encode("pool_xsk", bech32.toWords(Buffer.from(prvKeyHex, "hex")), 256); //encode in bech32 with a raised limit to 256 words because of the extralong privatekey (128bytes)

          //generate the public key json and bech format
          if (args["vkey-extended"] === true) {
            ...
            var pubKeyBech = bech32.encode("pool_xvk", bech32.toWords(Buffer.from(pubKeyHex, "hex")), 128); //encode in bech32 with a raised limit to 128 words because of the longer publickey (64bytes)
          } else {
      ...
      if (args["json"] === true) {
        ...
        var content = `{ "workMode": "${workMode}"`;
        if (derivation_path != "") {
          content += `, "derivationPath": "${derivation_path}"`;
        }
        if (derivation_type != "") {
          content += `, "derivationType": "${derivation_type}"`;
        }
        if (vote_purpose > -1) {
          content += `, "votePurpose": "${vote_purpose_description} (${vote_purpose})"`;
        }
        if (paperWalletMnemonics != "") {
          content += `, "paperWalletMnemonics": "${paperWalletMnemonics}"`;
        } //the original 27 words for a daedalus byron paper-wallet
        if (mnemonics != "") {
        ...
        if (rootKeyHex != "") {
          content += `, "rootKey": "${rootKeyHex}"`;
        }
        if (rootPubKeyHex != "") {
          content += `, "rootPublicKey": "${rootPubKeyHex}"`;
        }
        if (address != "") {
        ...
        if (XpubKeyHex != "") {
          content += `, "XpubKeyHex": "${XpubKeyHex}", "XpubKeyBech": "${XpubKeyBech}"`;
        }
        if (drepIdHex != "") {
          content += `, "drepIdHex": "${drepIdHex}", "drepIdBech": "${drepIdBech}", "drepIdHexLegacy": "${drepIdHexLegacy}", "drepIdBechLegacy": "${drepIdBechLegacy}"`;
        } else if (ccColdIdHex != "") {
          content += `, "ccColdIdHex": "${ccColdIdHex}", "ccColdIdBech": "${ccColdIdBech}"`;
        } else if (ccHotIdHex != "") {
          content += `, "ccHotIdHex": "${ccHotIdHex}", "ccHotIdBech": "${ccHotIdBech}"`;
        } else if (poolIdHex != "") {
          content += `, "poolIdHex": "${poolIdHex}", "poolIdBech": "${poolIdBech}"`;
        } else if (calidusIdHex != "") {
          content += `, "calidusIdHex": "${calidusIdHex}", "calidusIdBech": "${calidusIdBech}"`;
        }
        if (prvKeyBech != "") {
    ...
    case "canonize-cip100": //CANONIZE AND HASH JSONLD GOVERNANCE METADATA
      ...
      if (typeof data === "undefined" || data == "") {
        ...
        if (typeof data_file === "undefined" || data_file == "") {
          console.error(`Error: Missing data / data-file to hash`);
          showUsage(workMode);
        }
        ...
        try {
          ...
          console.error(`Error: Can't read data-file '${data_file}' or not valid JSON-LD data`);
          process.exit(1);
        }
      } else {
        ...
        try {
          ...
          console.error(`Error: Not valid JSON data (${error})`);
          process.exit(1);
        }
      }
      ...
      try {
        ...
        if (jsonld_data["body"] === undefined || jsonld_data["@context"] === undefined) {
          console.error(`Error: JSON-LD must contain '@context' and 'body' data`);
          process.exit(1);
        }
      } catch (error) {
        console.error(`Error: Couldn't extract '@context' and 'body' JSON-LD data (${error})`);
        process.exit(1);
      }

      //allow to canonized in an unsafe mode (dropping some data) if the --disable-safemode flag is set
      var safeMode = true;
      if (args["disable-safemode"] === true) {
        safeMode = false;
      }

      //Start the async canonize process, will get triggered via the .then part once the process finished
      jsonld
        .canonize(jsonld_data, { safe: safeMode, algorithm: "URDNA2015", format: "application/n-quads" })
        .then((canonized_data) => {
          ...
          if (args["json"] === true) {
            ...
            //generate content in extended json format
            ...
            var content =
              `{ "workMode": "${workMode}", "canonizedHash": "${canonized_hash}", "body": ` +
              JSON.stringify(jsonld_data["body"]) +
              `, "canonizedBody": [ ${canonized_array} ] }`;
          } else {
            //generate content in text format
        ...
        .catch((err) => {
          console.log(`Error: Could not canonize the data (${err.message})`);
          process.exit(1);
        });
    ...
    case "verify-cip100": //CANONIZE AND HASH JSONLD GOVERNANCE METADATA, CHECKS ALL THE AUTHORS SIGNATURES
      ...
      if (typeof data_file !== "undefined" && data_file != "") {
        ...
        //input was data provided via the --data parameter
        try {
      ...
      if (typeof data === "undefined" || data == "") {
        ...
        var data_file = args["data-file"];
        if (typeof data_file === "undefined" || data_file == "") {
          console.error(`Error: Missing data / data-file to verify`);
          showUsage(workMode);
        }
      ...
      try {
        if (jsonld_input["body"] === undefined || jsonld_input["@context"] === undefined) {
          console.error(`Error: JSON-LD must contain '@context' and 'body' data`);
          process.exit(1);
        }
      ...
      jsonld
        ...
        .then((canonized_data) => {
          ...
          //get the hash of the canonized data
          if (jsonld_input["hashAlgorithm"] != "blake2b-256") {
            console.error(`Error: unknown or missing hashAlgorithm - ${jsonld_input["hashAlgorithm"]}`);
            process.exit(1);
          }
          var canonized_hash = getHash(Buffer.from(canonized_data).toString("hex"));
          ...
          verifyAuthors: {
            ...
            if (jsonld_input["authors"] === undefined) {
              errorStr = "missing authors field";
              break verifyAuthors;
            }
            var jsonld_authors = jsonld_input["authors"];
            //check that the authors entry is an array
            if (typeof jsonld_authors !== "object" || jsonld_authors instanceof Array == false) {
              errorStr = "authors entry is not an array";
              break verifyAuthors;
            }
            //check that the number of authors is not zero
            if (jsonld_authors.length == 0) {
              errorStr = "no authors in the authors-array";
              break verifyAuthors;
            }
            //check each authors array entry
            jsonld_authors.every((authorEntry) => {
              var authorName = authorEntry["name"];
              if (typeof authorName !== "string") {
                errorStr = "authors.name entry is missing or not a string";
                return false;
              }

              var authorWitness = authorEntry["witness"];
              if (typeof authorWitness !== "object") {
                errorStr = "authors.witness entry is missing or not a json object";
                return false;
              }
              var authorWitnessAlgorithm = authorWitness["witnessAlgorithm"];
              if (typeof authorWitnessAlgorithm !== "string") {
                errorStr = "authors.witness entry is missing or not a string";
                return false;
              }

              var authorWitnessPublicKey = authorWitness["publicKey"];
              if (typeof authorWitnessPublicKey !== "string" || !regExpHex.test(authorWitnessPublicKey)) {
                errorStr = `authors.witness.publickey entry for '${authorName}' is not a valid hex-string`;
                return false;
              }
              //load the public key
              try {
                ...
                errorStr = `authors.witness.publickey entry for '${authorName}' error ${error}`;
                return false;
              }

              //check if there is a duplicated entry already for that public key
              var hasDuplicates = authors_array.some((element) => {
                return element["publicKey"] === authorWitnessPublicKey;
              });
              if (hasDuplicates) {
                errorStr = `authors.witness.publickey entry for '${authorName}' has duplicates`;
                return false;
              }

              var authorWitnessSignature = authorWitness["signature"];
              if (typeof authorWitnessSignature !== "string" || !regExpHex.test(authorWitnessSignature)) {
                errorStr = `authors.witness.signature entry for '${authorName}' is not a valid hex-string`;
                return false;
              }
              ...
              switch (authorWitnessAlgorithm) {
                case "ed25519": //witness is signed with the standard ed25519 algorithm
                  ...
                  try {
                    ...
                    errorStr = `authors.witness.signature entry for '${authorName}' error ${error}`;
                    return false;
                  }
                  //do the verification
                ...
                case "CIP-0030": //witness is signed with the CIP-0030 (signData) algorithm (deprecated)
                  ...
                  try {
                    var ret = verifyCIP8("verify-cip8", [
                      "--cip8",
                      "--cose-sign1",
                      authorWitnessSignature,
                      "--cose-key",
                      `a4010103272006215820${authorWitnessPublicKey}`, //prefix the publickey with the standard parameters for CIP0030
                      "--data-hex",
                      canonized_hash,
                    ]);
                  } catch (error) {
                    errorStr = `authors.witness.signature entry for '${authorName}' error ${error.msg}`;
                    return false;
                  }
                ...
                default: //no supported algorithm found or its missing
                  errorStr = `authors.witness.algorithm entry for '${authorName}' is missing or not supported. value='${authorWitnessAlgorithm}'`;
                  return false;
                  break;
              } //switch authorWitnessAlgorithm
              ...
              if (!verified) {
                errorStr = `at least one invalid signature found`;
              }

              //add it to the array of authors
              var authorArrayEntry = {
                name: authorName,
                valid: verified,
                algorithm: authorWitnessAlgorithm,
                publicKey: authorWitnessPublicKey,
                signature: authorWitnessSignature,
              };
          ...
          if (args["json"] === true) {
            //generate content in json format
            var content =
              `{ "result": ${result}, "errorMsg": "${errorStr}", "authors": ` +
              JSON.stringify(authors_array) +
              `, "fileHash": "${fileHash}" }`;
          } else if (args["json-extended"] === true) {
            //generate content in extended json format
            //split the canonized data into an array, remove the last element, do a loop for each element
            ...
            var content =
              `{ "workMode": "${workMode}", "result": ${result}, "errorMsg": "${errorStr}", "authors": ` +
              JSON.stringify(authors_array) +
              `, "canonizedHash": "${canonized_hash}", "fileHash": "${fileHash}", "body": ` +
              JSON.stringify(jsonld_data["body"]) +
              `, "canonizedBody": [ ${canonized_array} ] }`;
          } else {
            //generate content in text format
        ...
        .catch((err) => {
          console.log(`Error: Could not verify the data (${err.message})`);
          process.exit(1);
        });
    ...
    case "sign-cip100": //CANONIZE AND HASH JSONLD GOVERNANCE METADATA, SIGN THE DOCUMENT AND ADD AUTHORS
      ...
      if (typeof add_author_name === "undefined" || add_author_name === true || add_author_name == "") {
        console.error(`Error: Missing author name parameter`);
        showUsage(workMode);
      }

      //get an optional signing address/id. if present, use CIP-0008 signing mode instead of the default ed25519 mode
      var address = args["address"];
      if (typeof address !== "undefined" && address !== true) {
        authorSigningMode = "CIP-0008";
      }
      ...
      jsonld
        ...
        .then((canonized_data) => {
          ...
          //sign the data according to the choosen signing mode
          switch (authorSigningMode) {
            case "ed25519": //sign with standard ed25519 algorithm
              try {
                var signedBytes = prvKey.sign(Buffer.from(canonized_hash, "hex")).to_bytes();
                var signature = Buffer.from(signedBytes).toString("hex");
              } catch (error) {
                console.error(`Error: ed25519 signing error - ${error}`);
                process.exit(1);
              }
              break;

            case "CIP-0008": //sign with CIP-0008 algorithm, the given address/id must belong to the signing key
              try {
                var ret = signCIP8("sign-cip8", [
                  "--cip8",
                  "--data-hex",
            ...
            default: //no supported signingMode found or its missing
              console.error(
                `Error: authorSigningMode '${authorSigningMode}' is unsupported, you should not have landed here`,
              );
              process.exit(1);
              break;
          } //switch authorSigningMode
          ...
          verifyAuthors: {
            ...
            //check that the authors entry is an array
            if (typeof jsonld_authors !== "object" || jsonld_authors instanceof Array == false) {
              econsole.error(`Error: authors entry is not an array`);
              process.exit(1);
            }
            //check each authors array entry
            jsonld_authors.every((authorEntry) => {
              var authorName = authorEntry["name"];
              if (typeof authorName !== "string") {
                console.error(`Error: authors.name entry is not an string`);
                process.exit(1);
              }

              var authorWitness = authorEntry["witness"];
              if (typeof authorWitness !== "object") {
                console.error(`Error: authors.witness entry is missing or not a json object`);
                process.exit(1);
              }
              var authorWitnessAlgorithm = authorWitness["witnessAlgorithm"];
              if (typeof authorWitnessAlgorithm !== "string") {
                console.error(`Error: authors.witness.algorithm entry for '${authorName}' is missing or not a string`);
                process.exit(1);
              }

              var authorWitnessPublicKey = authorWitness["publicKey"];
              if (typeof authorWitnessPublicKey !== "string" || !regExpHex.test(authorWitnessPublicKey)) {
                console.error(`Error: authors.witness.publickey entry for '${authorName}' is not a valid hex-string`);
                process.exit(1);
              }
              //load the public key
              try {
              ...
              if (authorWitnessPublicKey == pubKey) {
                switch (args["replace"]) {
                  case true:
                    return true;
                    break; //publickey entry is the same, we wanna replace it with the new entry -> go to the next authors entry and don't add it to the existing list
                  case false:
                    console.error(
                      `Error: authors.witness.publickey entry for '${authorName}' is the same as the one you wanna add. please use the flag --replace if you wanna replace the entry`,
                    );
                    process.exit(1);
                    break;
                }
              ...
              if (hasDuplicates) {
                console.error(
                  `Error: authors.witness.publickey '${authorWitnessPublicKey}' in author '${authorName}' has duplicated entries! public keys must be unique, please remove duplicates first.`,
                );
                process.exit(1);
              }
              ...
              var authorWitnessSignature = authorWitness["signature"];
              if (typeof authorWitnessSignature !== "string" || !regExpHex.test(authorWitnessSignature)) {
                console.error(`Error: authors.witness.signature entry for '${authorName}' is not a valid hex-string`);
                process.exit(1);
              }
              ...
              switch (authorWitnessAlgorithm) {
                case "ed25519": //witness is signed with the standard ed25519 algorithm
                  ...
                  try {
                    ...
                    console.error(`Error: authors.witness.signature entry for '${authorName}' error ${error}`);
                    process.exit(1);
                  }
                  //do the verification
                  var verified = publicKey.verify(Buffer.from(canonized_hash, "hex"), ed25519signature);
                ...
                case "CIP-0008": //witness is signed with the CIP-0008 (signData) algorithm
                  ...
                  try {
                    ...
                    console.error(`Error: authors.witness.signature entry for '${authorName}' error ${error.msg}`);
                    process.exit(1);
                  }
                  var verified = ret.verified;
                  break;
                ...
                default: //no supported algorithm found or its missing
                  console.error(
                    `Error: authors.witness.algorithm entry for '${authorName}' is missing or not supported. value='${authorWitnessAlgorithm}'`,
                  );
                  process.exit(1);
                  break;
              } //switch authorWitnessAlgorithm
              ...
              var authorArrayEntry = {
                name: authorName,
                witness: {
                  witnessAlgorithm: authorWitnessAlgorithm,
                  publicKey: authorWitnessPublicKey,
                  signature: authorWitnessSignature,
                },
              };
          ...
          var authorArrayEntry = {
            ...
            witness: { witnessAlgorithm: authorSigningMode, publicKey: pubKey, signature: signature },
          };
          authors_array.push(authorArrayEntry);
          ...
          if (typeof out_file === "undefined" || out_file == "") {
            ...
            try {
              ...
              console.log(
                `{ "workMode": "${workMode}", "witnessAlgorithm": "${authorSigningMode}", "outFile": "${out_file}", "anchorHash": "${anchorHash}" }`,
              );
            } catch (error) {
              console.error(`${error}`);
              process.exit(1);
        ...
        .catch((err) => {
          console.log(`Error: Could not sign the data (${err.message})`);
          process.exit(1);
        });
    ...
    case "verify-cip88": //VERIFY DATA IN CIP88v2 (Calidus-Pool-Key)
      ...
      if (typeof data === "string" && data != "") {
        ...
        try {
          ...
          console.error(`Error: Not valid JSON data (${error}) provided via the --data parameter`);
          process.exit(1);
        }
      }
      ...
      if (!dataMap) {
        ...
        if (typeof data_file === "string" && data_file != "") {
          ...
          try {
            ...
            console.error(`Error: Can't read data-file '${data_file}' or not valid JSON data`);
            process.exit(1);
          }
        } //data-file
      } //dataMap
      ...
      if (!dataMap) {
        ...
        if (typeof data_hex === "string" && regExpHex.test(data_hex.toLowerCase())) {
          ...
          try {
            ...
            console.error(`Error: Can't cbor decode the given data-hex (${error})`);
            process.exit(1);
          }
        } //data-file
      } //dataMap
      ...
      if (!dataMap.has(867) || dataMap.get(867).size < 3) {
        console.error(`Error: Data to verify has no map key 867 or not enough entries`);
        process.exit(1);
      } else if (isNaN(dataMap.get(867).get(0)) || dataMap.get(867).get(0) < 2) {
        console.error(`Error: Map key 0 (version) is not set to 2 or above`);
        process.exit(1);
      } else if (!dataMap.get(867).has(1) || dataMap.get(867).get(1).size < 5) {
        console.error(`Error: Map key 1 (payload) is missing or does not have at least 5 entries`);
        process.exit(1);
      } else if (
        !dataMap.get(867).has(2) ||
        (!dataMap.get(867).get(2)) instanceof Array ||
        dataMap.get(867).get(2).length != 1
      ) {
        console.error(
          `Error: Map key 2 (witness) is missing or is not an array with a single entry (only one witness supported currently)`,
        );
        process.exit(1);
      }
      ...
      if (!payloadMap.has(1) || !payloadMap.has(2) || !payloadMap.has(3) || !payloadMap.has(4) || !payloadMap.has(7)) {
        console.error(`Error: Payload map does not have all needed keys 1,2,3,4 & 7`);
        process.exit(1);
      } else if ((!payloadMap.get(1)) instanceof Array || payloadMap.get(1)[0] != 1) {
        console.error(`Error: PayloadMap key 1 (registraction-scope) is not an array or not of type pool-scope(1)`);
        process.exit(1);
      } else if ((!payloadMap.get(3)) instanceof Array || isNaN(payloadMap.get(3)[0]) || payloadMap.get(3)[0] != 2) {
        console.error(`Error: PayloadMap key 3 (validation method) is not an array and/or not set to [2] -> CIP-0008`);
        process.exit(1);
      } else if (isNaN(payloadMap.get(4))) {
        console.error(`Error: PayloadMap key 4 (nonce) is not a uint number`);
        process.exit(1);
      } else if (!Buffer.isBuffer(payloadMap.get(7)) || payloadMap.get(7).length != 32) {
        console.error(`Error: PayloadMap key 7 (calidus-public-key) is not a hex bytearray/buffer of length 32`);
        process.exit(1);
      }
      ...
      if (!Buffer.isBuffer(scopePoolId) || scopePoolId.length != 28) {
        console.error(
          `Error: Pool-ID part in PayloadMap key 1 (registration-scope) is not a hex bytearray/buffer of length 28`,
        );
        process.exit(1);
      }
      ...
      try {
        ...
        console.error(`Error: PayloadMap key 7 (calidus-public-key) -> ${error}`);
        process.exit(1);
      }
      ...
      if (!witnessMap.has(1) || !witnessMap.has(2)) {
        console.error(`Error: Witness entry does not have the needed keys 1 & 2`);
        process.exit(1);
      } else if (witnessMap.has(0) && witnessMap.get(0) != 0) {
        console.error(`Error: Witness entry key 0 (Witness Type Identifier) is not 0(COSE_Witness)`);
        process.exit(1);
      }
      ...
      if (!Buffer.isBuffer(coseKeyMap.get(-2)) || coseKeyMap.get(-2).length != 32) {
        console.error(`Error: coseKeyMap key -2 (public key) is not a hex bytearray/buffer of length 32`);
        process.exit(1);
      }
      ...
      try {
        ...
        if (scopePoolIdHex != pubKeyPoolIdHex) {
          console.error(
            `Error: scope-PoolID(${scopePoolIdHex}) and witnessPublicKey-PoolID(${pubKeyPoolIdHex}) are not the same`,
          );
          process.exit(1);
        }
      } catch (error) {
        console.error(`Error: coseKeyMap key -2 (public key) -> ${error}`);
        process.exit(1);
      }

      //get the used poolId from the coseSign1Map[0]
      if (!Buffer.isBuffer(coseSign1Map[0]) || coseSign1Map[0].length != 41) {
        console.error(`Error: coseSign1Map[0] (protected header cbor) is not a hex bytearray/buffer of length 41`);
        process.exit(1);
      }
      try {
        var coseProtectedHeaderMap = cbor.decode(coseSign1Map[0]);
        var cosePoolIdHex = coseProtectedHeaderMap.get("address").toString("hex");
        if (scopePoolIdHex != cosePoolIdHex) {
          console.error(`Error: scope-PoolID(${scopePoolIdHex}) and cose-PoolID(${cosePoolIdHex}) are not the same`);
          process.exit(1);
        }
      } catch (error) {
        console.error(`Error: Can't cbor decode the given COSE_Sign1 protected header (${error})`);
        process.exit(1);
      }

      if (isNaN(coseSign1Map[1])) {
        console.error(`Error: coseSign1Map[1] (hashed true/false) is not a number`);
        process.exit(1);
      }
      switch (coseSign1Map[1]) {
        case 0:
          coseSign1Map[1] = { hashed: false };
          var verifyDataHex = payloadCborHash;
          break; //payload was not hashed
        case 1:
          coseSign1Map[1] = { hashed: true };
          var verifyDataHex = getHash(payloadCborHash);
          break; //payload was hashed, so we also hash our comparising message
        default:
          console.error(`Error: coseSign1Map[1] (hashed true/false) is not 0 or 1`);
          process.exit(1);
          break; //only 0 and 1 are supported,otherwise throw an error
      }
      ...
      try {
        var ret = verifyCIP8("verify-cip88", [
          "--cip8",
          "--cose-sign1",
          coseSign1Hex, //pass it as cbor encoded hex string
          "--cose-key",
          coseKeyHex, //pass it as cbor encoded hex string
          "--nohashcheck",
          "--json-extended", //we already did the hash check above
          "--data-hex",
          verifyDataHex,
        ]); //we provide the self generated data for checking and not the included one
      } catch (error) {
        console.error(`Error: CIP8 verification error ${error.msg}`);
        process.exit(1);
      }
      ...
      if (args["json"] === true) {
        ...
        if (args["include-maps"] === true) {
          //generate content also with JSON-Maps for the COSE_Key, COSE_Sign1 and verifyData structures
          content += `, "maps": { "COSE_Key": ${mapToJs(coseKeyMap)}, "COSE_Sign1": ${mapToJs(coseSign1Map)}, "payloadData": ${mapToJs(payloadMap)} }`;
        }
        content += ` }`;
      } else {
        //generate content in text format
